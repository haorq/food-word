...
import com.meiyuan.catering.core.util.dada.HttpClientUtils;
import com.meiyuan.catering.core.util.dada.config.AppConfig;
import com.meiyuan.catering.core.util.dada.config.AppConstant;
...
public class DadaRequestClient {
    ...
    public DadaApiResponse callRpc() {
        ...
        log.debug("达达入參：" + requestParams);
        try {
            String resp = HttpClientUtils.post(requestUrl, requestParams);
            return (DadaApiResponse) JsonUtil.fromJson(resp, DadaApiResponse.class);
        } catch (Exception e) {
